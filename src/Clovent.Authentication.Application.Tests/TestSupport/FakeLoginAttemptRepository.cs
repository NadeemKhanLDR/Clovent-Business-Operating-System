...
internal sealed class FakeLoginAttemptRepository : ILoginAttemptRepository
{
    ...
    public Task<IReadOnlyCollection<LoginAttempt>> GetRecentByIdentifierAsync(
        string attemptedIdentifier, DateTimeOffset sinceUtc, CancellationToken cancellationToken = default) =>
    ...
    public Task<IReadOnlyCollection<LoginAttempt>> GetRecentByUserIdAsync(
        UserId userId, DateTimeOffset sinceUtc, CancellationToken cancellationToken = default) =>
        Task.FromResult<IReadOnlyCollection<LoginAttempt>>(_attempts.Values
